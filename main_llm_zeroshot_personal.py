...
def main():
    ...
    def score_frontier(objs, target_desc):          #PersONAL : Assign a score to the frontier using a string describing valid objects and target object description
        query_str = "A room containing "
        for ob in objs:
            query_str += ob + ", "
        query_str += "and "
        query_str += target_desc + "."
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
            ...
            for e in range(num_scenes):
                ...
                for lay in range(tpm):
                    ...
                    if len(objs_list)>0 and found_goal[e] == 0:
                        ...
                        ## PersONAL : Change scoring method to use target description
                        frontier_score_list[e].append(score_frontier(objs_list, cname))
                    else:
        ...
        for e in range(num_scenes):
            ...
            cn = infos[e]['goal_cat_id'] + 4                    #PersONAL TODO : Does this need to be changed? This applies to all instances of the category, not the specific target instance
